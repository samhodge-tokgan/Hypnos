# Headless plugin-discovery check for the MEMatte OFX plugin.
#
# Run with (note OFX_PLUGIN_PATH — some Natron builds do not scan
# ~/Library/OFX/Plugins or ~/OFX/Plugins by default):
...
# The Trimap input must show up as a second clip — that is the structural
# difference from a single-input filter, so it is worth asserting at discovery.
if node is not None:
    try:
        labels = [node.getInputLabel(i) for i in range(node.getMaxInputCount())]
        print("INPUTS:", labels)
    except Exception as e:
        print("INPUTS_EXCEPTION:", repr(e))
